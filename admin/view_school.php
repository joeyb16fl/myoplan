...
<?
	...
	$_query="select * from reg_schools where id = $sc_id";
   ...
   <div class="panel-body1">
	<div class="table-responsive">
		<div class="col-md-6 widget_1_box2">
					...
					<p style="color:white"><?=$row['sc_phone']?></p>
		   	 	</div>
		   	 	<div class="wid_blog-desc">
		   	 		<div class="wid_blog-left">
		   	 			<img src="<?=$row["sc_logo"]?>" class="img-responsive" alt="">
		   	 		</div>
		   	 		<div class="wid_blog-right">
		   	 			<h2>Info</h2>
		   	 			<p>Email: <?=$row["sc_email"]?></p>
		   	 			<p>Password: <?=$row["sc_pwd"]?></p>
		   	 			<p>Registered Code: <?=$row["sc_reg_code"]?></p>
		   	 			<p>Documents Status: <?=($row["sc_doc_status"])? 'Completed':'Not Completed'?></p>
				...
				<form id="status_form" method="post" action="update_doc_status.php">
						...
						<div class="form-group">
							...
							<select onchange="document.getElementById('status_form').submit()" class="form-control1 ng-invalid ng-invalid-required ng-touched" name="doc_status">
								<option <?=($row['sc_doc_status'])?'selected':''?> value="0">Incomplete</option>
